...
export class Node extends Object {
	 ...
	 */
	getWorldBounds() {
		// 순환 참조 방지를 위해 사용처에서 모듈을 동적 임포트하거나 OBB/Rect 로직을 재구성합니다.
		// 여기서는 Rect 객체를 직접 다루는 대신 OBB나 단순 min/max를 반환하는 방식으로 작성하거나 Rect.js를 임포트해야 합니다.
		// Node 클래스 최상단에 Rect, OBB import를 추가하는 방식으로 진행하겠습니다.
		const worldCorners = this.getWorldCorners();
		...
		// Rect 임포트가 Node에 없으므로 Vector2 기반의 원시 데이터를 반환하거나 Rect 모듈을 임포트 해야합니다.
		// 임포트를 추가하겠습니다.
