...
// 튜토리얼3: 결합퍼즐 게임.
//==============================================================================
export class Tutorial_3 extends Scene {
	//==============================================================================
	// 멤버 변수 목록.
	//==============================================================================
	...
	/** @type { Vector2 } */ #dragOffset;
	/** @type { Vector2 } */ #startButtonPosition;
	/** @type { Vector2 } */ #startButtonSize;
	/** @type { number } */ #boardX;
	/** @type { number } */ #boardY;
	...
	async load(engine) {
		...
		this.#startButtonPosition = viewSize.multiply(0.5).add(Vector2.create(0, 400));
		this.#startButtonSize = Vector2.create(244, 92);
	...
	reset() {
		...
		this.#spawnTime = JELLY_SPWAN_TIME;
		...
		switch (this.#gameState) {
			...
			case GameState.playing: {
					...
					if (inputManager.isTouchMoved() && this.#draggedJelly) {
						this.#draggedJelly.node.setPosition(viewInputPosition.subtract(this.#dragOffset));
					}
					...
					if (inputManager.isTouchReleased() && this.#draggedJelly) {
						...
						if (targetJelly &&
							...
							targetJelly.node.setScale(Vector2.zero());
							...
							const tween = new Tween({ scale: 0 })
								...
								.setUpdate(v => {
									targetJelly.node.setScale(Vector2.one().multiply(v.scale));
								})
								.setComplete(() => {
						...
						// 이동.
							...
							const newPos = this.gridToWorldPosition(releasedJelly.point);

							releasedJelly.isAnimating = true;

							const tween = new Tween({ x: releasedJelly.node.getPosition().x, y: releasedJelly.node.getPosition().y })
								.to({ x: newPos.x, y: newPos.y }, 0.2)
								.easing(Tween.easingFunction.back.out)
								.setUpdate(v => {
									releasedJelly.node.setPosition(Vector2.create(v.x, v.y));
								})
								.setComplete(() => {
	...
	createRandomJelly() {
		...
		for (let x = 0; x < GRID_COLUMNS; ++x) {
			for (let y = 0; y < GRID_ROWS; ++y) {
				if (!this.#grid[x][y]) {
					emptyCells.push(Vector2.create(x, y));
				}
			}
		}
		...
		jelly.node.setScale(Vector2.zero());
		...
		const tween = new Tween({ scale: 0 })
			...
			.easing(Tween.easingFunction.back.out)
			.setUpdate(v => {
				jelly.node.setScale(Vector2.one().multiply(v.scale));
			})
			.setComplete(() => {
...
document.title = "vanilla.js - Tutorial_3";
const tutorial = new Tutorial_3();
engine.run(tutorial);
